...
// snapshot::snapshot(std::string &PATH_) : PATH(PATH_) {}

// std::string snapshot::generate(std::vector<cv::Mat> &frames, std::vector<std::string> &text_lines)
std::string generate(std::vector<cv::Mat> &frames, std::vector<std::string> &text_lines)
{
	...
	// std::string filename = datetime.datetime.utcnow().strftime("%Y%m%dT%H%M%S.jpg");
